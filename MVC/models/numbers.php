...
class Numbers extends Model
{
    ...
    public static function getNumbers(){
	    global $db;
		$stmt = $db->prepare("
			    SELECT numbers.`id`, numbers.`number`, numbers.`price`, numbers.`country_id`, numbers.`agregator_id`, countries.`name` AS `country`, agregators.`name` AS `agregator` 
    ...
    public static function addNumber(){
        ...
        if($stmt->rowCount()==0){
            $data = array(
            $_POST['number'],$_POST['price']*100,$_POST['agregator'],$_POST['country']
            );  
            $stmt = $db->prepare('
			    INSERT INTO `numbers`(`number`, `price`, `agregator_id`, `country_id`) VALUES (?,?,?,?)
		    ');
            $stmt->execute($data);
        }
    ...
    public static function saveNumber(){
	    ...
	    global $db;
		$data = array(
            $_POST['price']*100,$_POST['agregator'],$_POST['country'],$_POST['number'],$_POST['number']
            );  
            $stmt = $db->prepare('
			    UPDATE `numbers` SET `price`=?,`agregator_id`=?,`country_id`=?,`number`=? WHERE `number`=?
		    ');
            $stmt->execute($data);
	}
